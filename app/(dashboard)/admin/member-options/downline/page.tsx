...
export default function MemberDownlinePage() {
    ...
    const handleFetchDownline = async (e: React.FormEvent) => {
        ...
        try {
            const res = await getMemberTreeAPI(downlineId);
            if (res.data?.status && res.data.data) {
                const apiData = res.data.data;
                const members: DownlineRow[] = [];
                
                // Add the downlines to the table
                if (apiData.downlines && Array.isArray(apiData.downlines)) {
                    members.push(...apiData.downlines);
                }
                
                setDownlineData(members);
                if (members.length === 0) {
                    toast("No downline members found", { icon: "ℹ️" });
                } else {
                    toast.success("Downline records retrieved");
                }
            } else {
                toast.error(res.data?.message || "Failed to fetch downline");
            }
        } catch (error: any) {
            console.error("Fetch downline error:", error);
            // Error toast usually handled by interceptor, but we log for debugging
        } finally {
            setIsLoading(false);
        }
    };
                                    ...
                                    <div className="flex items-center gap-10">
                                        <div className="relative group min-w-[300px]">
                                            ...
                                            <Input
                                                ...
                                                className="h-8 w-full rounded-md border border-gray-200 bg-white pl-8 pr-2 text-xs shadow-sm transition focus:border-primary focus:ring-1 focus:ring-primary/20"
                                            />
                                        </div>
                                        ...
                                        <Button
                                            type="submit"
                                            disabled={isLoading}
                                            className="h-8 px-3 text-xs rounded-md bg-primary text-white font-medium shadow-sm hover:bg-primary/90"
                                        >
